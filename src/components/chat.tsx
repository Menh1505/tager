...
const Chat = ({ workspaceId }: ChatProps) => {
  ...
  const { messages, sendMessage } = useSocketChat({ workspaceId }); // Truyền workspaceId vào hook
          ...
          <div className="flex flex-col h-full">
                ...
                <div key={index} className={`mb-3 flex ${msg.userId === user.$id ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[70%] px-3 py-2 rounded-lg ${
                      msg.userId === user.$id ? "bg-blue-500 text-white rounded-tr-none" : "bg-gray-200 text-gray-800 rounded-tl-none"
                    }`}>
                    {msg.userId !== user.$id && <div className="text-xs font-semibold mb-1">{msg.userName}</div>}
                    <div>{msg.content}</div>
                    <div className={`text-xs mt-1 ${msg.userId === user.$id ? "text-blue-100" : "text-gray-500"}`}>{formatTime(msg.timestamp)}</div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-3 bg-white border-t">
              <div className="flex items-center gap-2">
                ...
                />
                <button className="bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-600 transition" onClick={handleSend}>
                  Gửi
                </button>
              </div>
            </div>
